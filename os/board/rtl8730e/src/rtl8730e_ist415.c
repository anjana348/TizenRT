...
struct rtl8730e_ist415_touchinfo_s {
	struct touch_lower_s touch_config;
...
static void rtl8730e_ist415_irq_attach(touch_handler_t handler, FAR char *arg)
{
	...
	gpio_irq_init(&g_ist415info.data_ready, IST415_GPIO_I2C_PIN, rtl8730e_ist415_irq_handler, (uint32_t)arg);
...
void rtl8730e_ist415_initialize(void)
{
	...
	struct touch_dev_s *dev = (struct touch_dev_s *)ist415_touchinitialize(i2c, &(g_ist415info.touch_config));

	if (!dev || touch_register(dev) < 0) {		// Register touch driver
