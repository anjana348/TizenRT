...
static int touch_open(FAR struct file *filep)
{
	...
	if (priv->crefs == 0) {
		priv->dev->touch_enable(priv->dev);
	}
	priv->crefs++;
...
static int touch_close(FAR struct file *filep)
{
	...
	priv->crefs--;
	if (priv->crefs == 0) {
		priv->dev->touch_disable(priv->dev);
#ifndef CONFIG_DISABLE_POLL
...
static int touch_poll(FAR struct file *filep, FAR struct pollfd *fds, bool setup)
{
	FAR struct touch_s *priv = filep->f_inode->i_private;
	...
	if (setup) {
		...
		eventset = 0;
		touch_semtake(&priv->sem, false);
		pending = priv->dev->is_touchSet(priv->dev);
		...
		if (eventset) {
			touch_pollnotify((struct touch_dev_s *)&priv->dev, POLLIN);
		}
...
void touch_notify(struct touch_dev_s *priv)
{
...
{
	g_dev = (struct touch_s *)kmm_zalloc(sizeof(struct touch_s));
	if (!g_dev) {
		touchdbg("ERROR: Memory allocation failed\n");
		return -ENOMEM;
	}
	sem_init(&g_dev->sem, 0, 1);
	sem_init(&g_dev->pollsem, 0, 1);
	g_dev->dev = dev;
	g_dev->dev->notify_touch = touch_notify;
#ifndef CONFIG_DISABLE_POLL
	(void)touch_semtake(&g_dev->pollsem, false);
	for (int i = 0; i < CONFIG_TOUCH_NPOLLWAITERS; i++) {
			g_dev->fds[i] = NULL;
	}
	touch_semgive(&g_dev->pollsem);
#endif
	int ret = register_driver(TOUCH_DEV_PATH, &g_touchdev_fileops, 0666, g_dev);
	if (ret < 0) {
		kmm_free(g_dev);
		sem_destroy(&g_dev->sem);
		sem_destroy(&g_dev->pollsem);
